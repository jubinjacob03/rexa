...
function parseKnowledgeBase(filePath) {
  ...
  while ((match = sectionRegex.exec(content)) !== null) {
    ...
    // Skip the intro section (first one without proper title)
    if (title && body && title !== 'SHANTHA AI ASSISTANT - KNOWLEDGE BASE FOR RAG') {
      sections.push({
        title,
        content: `# ${title}\n\n${body}`,
      });
...
function getCategoryAndTags(title) {
  ...
  if (titleLower.includes('tool')) {
    category = 'tools';
    tags.push('tools', 'api');
  } else if (titleLower.includes('decision') || titleLower.includes('scenario')) {
  ...
  } else if (titleLower.includes('server') || titleLower.includes('permission')) {
    category = 'server';
    tags.push('server', 'permissions', 'roles');
  } else if (titleLower.includes('community') || titleLower.includes('gaming')) {
    category = 'community';
...
function chunkSection(section, maxChunkSize = 3000) {
  ...
  console.log(`  Section "${section.title}" is large (${section.content.length} chars), splitting...`);
  ...
  for (const subsection of subsections) {
    if (currentChunk.length + subsection.length > maxChunkSize && currentChunk.length > 100) {
      chunks.push({
...
async function ingestKnowledgeBase() {
  try {
    ...
    // Path to knowledge base file (in parent directory)
    const knowledgeBasePath = join(__dirname, '../../SHANTHA_KNOWLEDGE_BASE.txt');
